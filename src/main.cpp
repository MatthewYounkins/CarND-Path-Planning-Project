...
int NextWaypoint(double x, double y, double theta, vector<double> maps_x, vector<double> maps_y, vector<double> maps_dx, vector<double> maps_dy)
{
	...
	// increment closestWaypoint, otherwise we are beyond the waypoint and we need to increment
	
	double inner = hx*vx+hy*vy;
	if (inner<0.0) {
		closestWaypoint++;
	}
	// here there used to be an angle check to increment closest waypoint... MY
	return closestWaypoint;
}
...
vector<double> getXY(double s, double d, vector<double> maps_s, vector<double> maps_x, vector<double> maps_y)
{
	...
	double perp_heading = heading-0.5*pi();
          ...
          // double car_x = j[1]["x"];
          	...
          	double end_path_d = j[1]["end_path_d"];
						...
						*/
						...
						if (prev_size < 2)
						{
							//Use two points that make the path tangent to teh car
						...
						// Fill up the rest of our points after filling it with previous points, here we will always allow for  50 points
